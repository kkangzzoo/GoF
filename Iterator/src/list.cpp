...
int AbstractList::Count(){ return totalCnt_; }
...
void LinkedList::AddNext(Item* pNewItem, Item* pItem){
	...
	if(pFirst_==0)	pFirst_=new LinkedItem(pNewItem);
...
void TreeList::AddNext(Item* pNewItem, Item* pItem){
	if(pRoot_==0)	pRoot_=new TreeItem(pNewItem); //처음 등록
	else if(pItem==0 || pItem==pRoot_->pData_){ //root 항목의 첫번째 자식으로 삽입
		TreeItem* pTmp=pRoot_->pFirstChild_;
		pRoot_->pFirstChild_=new TreeItem(pNewItem, pRoot_, 0, pTmp);
	}else{ //pData_값이 pItem과 같은 항목의 형제 노드로 삽입
		TreeItem* pPrev=0;
		...
		}else{ //같은 노드가 없으면 맨 마지막의 노드의 형제로 삽입
...
void TreeList::AddChild(Item* pNewItem, Item* pItem){
	if(pRoot_==0)	pRoot_=new TreeItem(pNewItem);
		else if(pItem==0 || pItem==pRoot_->pData_){ //root 항목의 첫번째 자식으로 삽입
			TreeItem* pTmp=pRoot_->pFirstChild_;
			pRoot_->pFirstChild_=new TreeItem(pNewItem, pRoot_, 0, pTmp);
		}else{ //pData_값이 pItem과 같은 항목의 형제 노드로 삽입
...
TreeList::TreeItem* TreeList::GetNext(const TreeItem* pStart){
	...
	else if(pStart->pSibling_!=0)	return pStart->pSibling_; //형제노드 존재시
	else if(pStart->pParent_ ==0) return pStart->pFirstChild_; //Root 노드 경우
	else return pStart->pParent_->pFirstChild_->pFirstChild_; //현재 레벨에서 마지막 형제노드인 경우
}
