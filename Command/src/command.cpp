...
public:
	bool GetRequest(string input, Request& req){
		...
		while(pos!=NA_POS){
			...
			pos=input.find("&",start);
			if(pos==NA_POS)	value=input.substr(start, input.length()-start);
			else value = input.substr(start, pos-start);
			start=pos+1;
...
int main() {
	...
	while(1){
		...
		else cout<<"Not Available Command:"<<cmd<<endl;
	}
	return 0;
}
