...
void Mixer::SetMediator(Mediator* pMediator){
...
void Mixer::AddCup(int addCnt) {
	...
	if (mixLevel_ < MIN_MIX_LEVEL && oldCupCnt < MIN_CUP_CNT
			&& cupCnt_ >= MIN_CUP_CNT)
		pMediator_->InformResourceState(RESOURCE_OK);
}

int Mixer::TakeOut(int wantedCupCnt){
	if (!IsValid())
			return 0;
...
void Mixer::OutOfOrder(){
	pMediator_->InformResourceState(MIXER_OUT_OF_ORDER);
}
void Mixer::Repair(){
	pMediator_->InfromMixerState(MIXER_OK);
}
