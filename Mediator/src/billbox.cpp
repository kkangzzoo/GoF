...
void BillBox::SetMediator(Mediator* pMediator){
	pMediator_=pMediator;
}

void BillBox::InsertBill(int wantedCupCnt) {
	if (!IsValid() || pMediator_->IsCoinLack() || pMediator_->IsBillFull()
	...
	if(wantedCupCnt > servedCnt) cout<<"Sorry. No More Served"<<endl;

	if(billCnt_>=MAX_BILL_CNT) pMediator_->InformBillState(BILL_FULL);
}
...
void BillBox::TakeOutBill(int takeOutCnt){
	...
	if(billCnt_<MAX_BILL_CNT)
		pMediator_->InformResourceState(BILL_NOT_FULL);
}

void BillBox::SetBillBoxMode(int mode){
	mode_=mode;
}
